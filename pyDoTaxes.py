...
img = ImageGrab.grab()
size = img.size
x1 = size[0] / 2 - 300; y1 = size[1] / 2 - 300
x2 = x1 + 700; y2 = y1 + 700 # these values needs to be double the values above to hit the center of screen. Offset using these
portion = img.crop((x1, y1, x2, y2))
while (True):
    # cap = np.array(ImageGrab.grab()) #capture full desktope
    ...
    if min_val < threshold: #this is to determine the threshold, I figured this out by adding print(min_val) in the loop and seeing what numbers it printed when it was recognized
        ...
        print("Hook Detected!" + str(min_val))
        reelpause = randint(0, 2.0)
